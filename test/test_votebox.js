...
contract('votebox', accounts => {
    ...
    beforeEach(async () => {
        ...
        shub = await TestToken.new("ShitHub", "SHUB");
        voteBox = await VoteBox.new(shub.address);
    });
    ...
    describe("propose", async () => {
        ...
        it("insufficient shub", async () => {
            await shub.mint(u1, '999999' + '999999999999999999');
            try {
                const { beginBlock, endBlock } = await defaultActiveBlock();
                await voteBox.propose("https://", beginBlock, endBlock, { from: u1 });
                throw null;
        ...
        describe("with proposal privilege", async () => {
            beforeEach(async () => {
                await shub.mint(u1, toWad('1000000'));
            });
